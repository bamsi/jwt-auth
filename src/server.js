...
app.post('/api/superset-guest-token', (req, res) => {
  ...
  if (!user || typeof user !== 'object' || !user.username) {
    return res.status(400).json({ message: 'Invalid request: "user" object with "username" is required.' });
  }
  if (!resources || !Array.isArray(resources)) {
    return res.status(400).json({ message: 'Invalid request: "resources" array is required.' });
  }
  // rls_rules is optional, but if provided, should be an array
  if (rlsRules && !Array.isArray(rlsRules)) {
    return res.status(400).json({ message: 'Invalid request: "rls_rules" if provided, must be an array.' });
  }

  // Check if SUPERSET_SECRET_KEY is configured
  if (!config.SUPERSET_SECRET_KEY || config.SUPERSET_SECRET_KEY === 'YOUR_SUPERSET_GUEST_TOKEN_SECRET_KEY_GOES_HERE') {
    console.error('JWT generation failed: SUPERSET_SECRET_KEY is not configured or is still set to the placeholder.');
    return res.status(500).json({ message: 'Server configuration error: Secret key not set.' });
  }
  ...
  try {
    const token = jwt.sign(payload, config.SUPERSET_SECRET_KEY, { algorithm: 'HS256' });
    res.json({ token });
  } catch (error) {
...
app.listen(port, () => {
  ...
  console.log('SUPERSET_SECRET_KEY:', config.SUPERSET_SECRET_KEY && config.SUPERSET_SECRET_KEY !== 'YOUR_SUPERSET_GUEST_TOKEN_SECRET_KEY_GOES_HERE' ? 'Set (masked)' : 'Not Set or Placeholder');
